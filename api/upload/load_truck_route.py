...
class LoadTruckRoute:
    ...
    def load_truck_route(self, truck_route_file):
        ...
        try:
            for data in truck_route_file[1:-1]:
                ...
                try:
                    ...
                    if not Truck.objects.filter(truck_number=truck_route["truck_number"]):
                        raise ValueError("Truck number {} does not exist".format(truck_route["truck_number"]))
                    #Check if the route number exists
                    if not Route.objects.filter(route_number=truck_route["route_number"]):
                        raise ValueError("Route number {} does not exist".format(truck_route["route_number"]))
                    try:
            ...
            log.error(str(e))
            errors["data"].append("Please check the formatting of the file")

        log.info("Updated Truck Routes")
